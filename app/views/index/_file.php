...
<tr <? if ($full_access) printf('data-file="%s"', $file_ref->id) ?>
    ...
    <td class="document-icon" data-sort-value="<?= crc32($file_ref->mime_type) ?>">
        <? if ($current_folder->isFileDownloadable($file_ref, $GLOBALS['user']->id)) : ?>
            <a href="<?= htmlReady($file_ref->download_url) ?>" target="_blank">
                <?= Icon::create(FileManager::getIconNameForMimeType($file_ref->mime_type))->asImg(24) ?>
            </a>
        <? else : ?>
            <?= Icon::create(FileManager::getIconNameForMimeType($file_ref->mime_type), Icon::ROLE_INACTIVE)->asImg(24) ?>
        <? endif ?>
    </td>
    ...
    <td title="<?= number_format($file_ref->size, 0, ',', '.') . ' Byte' ?>" data-sort-value="<?= $file_ref->size ?>"
    ...
    <td title="<?= strftime('%x %X', $file_ref->chdate) ?>" data-sort-value="<?= $file_ref->chdate ?>"
        class="responsive-hidden">
